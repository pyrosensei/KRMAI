# 🧠 KRMAI — RAG Implementation Pipeline

## Project Overview

**KRMAI** is a local LLM-based Knowledge Retrieval System that uses **Retrieval-Augmented Generation (RAG)** to provide accurate, context-grounded answers to student queries about fees, placements, hostels, scholarships, and other university topics.

Instead of relying solely on the LLM's training data, KRMAI retrieves relevant document chunks from a local vector database and feeds them as context, ensuring factual and hallucination-free responses.
...
├── app.py                         # 🖥️ Streamlit UI (legacy/alternative frontend)
│
...
User Question: "what's the hostel fee bro?"
   ...
   └──────┬───────┘
          ...
          ▼
   ┌──────────────┐
   │  LLM INVOKE  │  Ollama → qwen2.5:3b (local inference)
...
| LLM | Ollama + qwen2.5:3b | Fully offline, no API keys, 3B params is fast on CPU |
...
|-----------|-----------|---------|
| **LLM** | Ollama + qwen2.5:3b | Local language model inference |
...
Slang Expansion:   "what is the hostel fee for single room?"
                    ↓
ChromaDB Search:   Retrieves chunks from krmu_hostel.txt & krmu_fee_structure.txt
                    ↓
LLM Prompt:        Context + Question → qwen2.5:3b
                    ↓
Response:          "The single room hostel fee is Rs. 15,000 per semester."
                    Sources: krmu_hostel.txt, krmu_fee_structure.txt
```

---

*Built by Team KRMAI — LLM-Based Knowledge Retrieval System*
